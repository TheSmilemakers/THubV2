...
export function LoadingSequence({ onComplete }: LoadingSequenceProps) {
    ...
    const timer = setInterval(() => {
      setProgress((prev) => {
        ...
        if (newProgress >= 100) {
          clearInterval(timer);
          setIsComplete(true);
          timeoutId = setTimeout(() => onComplete?.(), 300); // Small delay for visual effect
          return 100;
        }
  ...
  return (
    <div className="mb-8 space-y-4" role="status" aria-live="polite" aria-label="System initialization progress">
      {/* Loading Steps */}
        ...
        {loadingSteps.map((step, index) => (
          <div
            ...
            className={cn(
              ...
              theme === 'synthwave' ? "font-mono" : "font-medium",
              index < currentStep 
                ? theme === 'synthwave' ? "text-terminal-green opacity-100" : "text-primary opacity-100"
                : index === currentStep
                  ? theme === 'synthwave' ? "text-neon-cyan opacity-100" : "text-secondary opacity-100"
                  : theme === 'synthwave' ? "text-neon-purple opacity-30" : "text-text-muted opacity-50"
            )}
          >
            <span className={theme === 'synthwave' ? "text-neon-cyan" : "text-primary"}>
              {theme === 'synthwave' ? '>' : '•'}
            </span>
            <span className="flex-1">{step}</span>
            {index < currentStep && (
              <span className={theme === 'synthwave' ? "text-terminal-green" : "text-primary"}>
                {theme === 'synthwave' ? '[✓]' : '✓'}
              </span>
            )}
      ...
      <div className="space-y-2">
        ...
        <div 
          className="relative h-2 bg-bg-tertiary border border-neon-pink/30 rounded overflow-hidden"
          role="progressbar" 
      ...
      {/* System Status */}
      {isComplete && (
        <div className="mt-6 p-4 border border-terminal-green/50 rounded bg-terminal-green/5" role="alert">
          <div className="flex items-center gap-3">
            <div className="w-3 h-3 bg-terminal-green rounded-full animate-pulse" aria-hidden="true" />
