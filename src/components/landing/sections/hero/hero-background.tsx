...
export function HeroBackground({ className }: HeroBackgroundProps) {
  ...
  const shouldRenderCanvas = () => {
    ...
    const hasVeryLowMemory = 'deviceMemory' in navigator && (navigator as any).deviceMemory < 2;
    
    // Always render unless device has very low memory
    return !hasVeryLowMemory;
  };
  ...
  useEffect(() => {
    // Skip on server and before hydration
    if (!isHydrated || !shouldRenderCanvas()) {
      return;
    }
    ...
    const startAnimationWhenReady = () => {
      ...
      if (typeof ResizeObserver !== 'undefined') {
        const observer = new ResizeObserver((entries) => {
          ...
          // Canvas is now in DOM with dimensions
          observer.disconnect();
        ...
        const checkAndStart = () => {
          if (canvas.offsetWidth > 0 && canvas.offsetHeight > 0 && !animationStarted) {
            animationStarted = true;
            const ctx = contextManager.getContext(canvas, contextIdRef.current);
            if (!ctx) {
              return;
        ...
        checkAndStart();
        const timer = setTimeout(checkAndStart, 100);
        cleanupObserver = () => clearTimeout(timer);
      }
    };
  ...
  }, [theme, contextManager, isHydrated]);

  // Always render canvas without conditions for synthwave
  const renderCanvas = theme === 'synthwave' || (isHydrated && shouldRenderCanvas());
  ...
  return (
    <div className={cn("absolute inset-0 overflow-hidden", className)}>
      ...
      {/* CSS-based pulsating lines for synthwave theme */}
      {theme === 'synthwave' && (
        <>
          {/* Pulsating red geometric lines */}
          <div className="absolute inset-0 overflow-hidden">
            {/* Horizontal pulsating lines */}
            ...
            <div className="absolute inset-0" style={{ perspective: '1000px' }}>
              {[...Array(12)].map((_, i) => (
                <div
                  ...
                  style={{
                    left: `${10 + i * 7}%`,
                    background: `linear-gradient(to bottom, transparent, rgba(255, 0, 110, ${0.8 - i * 0.05}), transparent)`,
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-bg-primary via-transparent to-bg-primary/30 pointer-events-none" />
        </>
      )}
      
      {/* Additional static overlays - only render after mount to avoid hydration mismatch */}
      
      {isHydrated && theme === 'professional' && (
